...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        m_LED.setDefaultCommand(new RunCommand(() -> {
            ...
            double targetAngleInForTurretPosition = m_turret.getAngle() - m_visionController.getFilteredYaw();
            boolean isTargetOutsideLimits = hasValidTarget && !m_turret.isAngleGood(targetAngleInForTurretPosition);
    ...
    private void configButtonBindings() {
        ...
        coController.leftTrigger.whenPressed(
                new SequentialCommandGroup(new RunCommand(m_intake::retractInner, m_intake).withTimeout(.5),
        ...
        coController.leftBumper.and(coController.leftTrigger).whenActive(
                new ParallelCommandGroup(new GroundIntake(m_intake), new LoadBalls(m_indexer, m_ballStopper)));

        coController.leftBumper.whenReleased(new ParallelCommandGroup(new RunCommand(m_intake::end, m_intake),
        ...
        coController.rightBumper.whenActive(new ParallelCommandGroup(
                ...
                new AccelerateFlywheelKickerWheel(m_flywheel, m_kickerWheel, this::getFlywheelRPM, m_visionController::isValid)));
        ...
        coController.buttonB
                .whenActive(new ParallelCommandGroup(new MoveHood(m_hood, Constants.cHood.trenchShotPosition),
    ...
    public Command getAutonomousCommand() {
        RamseteCommand initiationLineToTrench = new RamseteCommand(Trajectories.initiationLineToTrenchBalls,
        ...
        RamseteCommand trenchToCenterTower = new RamseteCommand(Trajectories.trenchBall3ToFrontOfTower,
                m_drivetrain::getPose, new RamseteController(), Constants.cDrivetrain.kDriveKinematics,
                m_drivetrain::setVelocityMpS, m_drivetrain);

        Command sequence = new SequentialCommandGroup(new TurretMotionMagic(m_turret, 30).withTimeout(.4),
                new AutoAimTurretHood(m_hood, m_turret, this::getHoodPosition, m_visionController::getFilteredYaw,
                        m_visionController::isValid).withTimeout(.3),
                new ShootClosedLoop(m_flywheel, m_kickerWheel, m_indexer, m_ballStopper, this::getFlywheelRPM,
                        Constants.cKickerWheel::getFlywheelOutputFromFlywheelRPM, IndexerSignal.GO_FAST).withTimeout(3),
                ...
                new ParallelDeadlineGroup(trenchToCenterTower,
                        new SequentialCommandGroup(
                                new StowIntakeAndOrganizeFeeder(m_intake, m_indexer, m_kickerWheel).withTimeout(2),
                ...
                new RunCommand(this::stopDrivetrain, m_drivetrain).withTimeout(.1),
                new ShootClosedLoop(m_flywheel, m_kickerWheel, m_indexer, m_ballStopper, this::getFlywheelRPM,
                        Constants.cKickerWheel::getFlywheelOutputFromFlywheelRPM, IndexerSignal.GO_FAST).withTimeout(4),
                new StopShooting(m_flywheel, m_kickerWheel, m_indexer));
        return sequence;
    }
